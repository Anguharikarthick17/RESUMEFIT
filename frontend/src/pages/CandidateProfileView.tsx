      ...
      <div className="dash-card p-6 sm:p-8 bg-white space-y-4">
        <div className="flex items-center justify-between">
          ...
          {candidate?.resume_filename && (
            <span className="text-xs font-mono text-emerald-800 bg-emerald-50 border border-emerald-200 px-2.5 py-0.5 rounded font-bold">
              Active: {candidate.resume_filename}
            </span>
          )}
        </div>

        <label className="dash-card p-8 border-2 border-dashed border-[#D4D4D4] hover:border-black bg-[#F8F8F7] hover:bg-[#F2F2F2] cursor-pointer transition-all flex flex-col items-center justify-center text-center space-y-3 block">
          <div className="w-12 h-12 rounded-xl bg-[#F5F5F4] border border-[#E5E5E5] flex items-center justify-center text-[#111111]">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="dash-card p-6 bg-white space-y-4">
            <div className="flex items-center gap-3 pb-3 border-b border-[#E5E5E5]">
              <div className="w-12 h-12 rounded-xl bg-black text-white font-black text-lg flex items-center justify-center border border-neutral-800">
                {candidate.name.charAt(0)}
              </div>
              <div>
                <h3 className="text-base font-bold text-[#111111]">{candidate.name}</h3>
                <p className="text-xs text-[#777777] font-mono">{candidate.summary || 'Applicant'}</p>
              </div>
            </div>

            <div className="space-y-2.5 text-xs text-[#555555] font-medium">
              {candidate.email && (
              ...
              {candidate.linkedin_url && (
                <div className="flex items-center gap-2">
                  ...
                  <a href={candidate.linkedin_url} target="_blank" rel="noreferrer" className="text-[#111111] hover:underline truncate font-semibold">
                    {candidate.linkedin_url}
                  </a>
                </div>
              )}
              ...
              <div className="pt-3 border-t border-[#E5E5E5] space-y-2">
                ...
                <div className="flex flex-wrap gap-1.5">
                  {candidate.skills.map((s, idx) => (
                    <span key={idx} className="px-2 py-0.5 rounded text-[11px] font-semibold bg-[#F5F5F4] text-[#111111] border border-[#E5E5E5]">
                      {s}
                    </span>
                  ))}
                </div>
          ...
          <div className="lg:col-span-2 space-y-6">
            ...
            <div className="dash-card p-6 bg-white space-y-3">
              <h4 className="text-xs font-bold font-mono text-[#111111] uppercase flex items-center gap-2">
              ...
              {candidate.experience && candidate.experience.length > 0 ? (
                <div className="space-y-2">
                  {candidate.experience.map((exp, idx) => (
                    <div key={idx} className="p-3 bg-[#F8F8F7] rounded-lg border border-[#E5E5E5] text-xs">
                      <span className="font-bold text-[#111111]">{exp.role || 'Role'}</span>
                      {exp.evidence && <p className="text-[#555555] font-sans mt-1">{exp.evidence}</p>}
                    </div>
                  ))}
                </div>
              ) : (
                <p className="text-xs text-[#888888] italic">No structured roles extracted.</p>
              )}
            </div>
              ...
              {candidate.education && candidate.education.length > 0 ? (
                <div className="space-y-2">
                  {candidate.education.map((edu, idx) => (
                    <div key={idx} className="p-3 bg-[#F8F8F7] rounded-lg border border-[#E5E5E5] text-xs">
                      <span className="font-bold text-[#111111]">{edu.degree || 'Degree'}</span>
                      {edu.evidence && <p className="text-[#555555] font-sans mt-1">{edu.evidence}</p>}
                    </div>
                  ))}
                </div>
              ) : (
                <p className="text-xs text-[#888888] italic">No education degree detected.</p>
              )}
            </div>
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
              <div className="dash-card p-4 bg-white space-y-2">
                <h4 className="text-xs font-bold font-mono text-[#111111] uppercase flex items-center gap-2">
                  <Code size={13} className="text-[#111111]" />
                  <span>Projects</span>
                </h4>
                {candidate.projects && candidate.projects.length > 0 ? (
                  ...
                  <p className="text-[11px] text-[#888888] italic">None listed</p>
                )}
              </div>

              <div className="dash-card p-4 bg-white space-y-2">
                <h4 className="text-xs font-bold font-mono text-[#111111] uppercase flex items-center gap-2">
                  <Award size={13} className="text-[#111111]" />
